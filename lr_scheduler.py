...
class NoamScheduler(LRScheduler):
    ...
    def __init__(
        ...
        # Parent __init__ must come AFTER setting our attributes because
        # it immediately calls get_lr() to initialise the param groups.
    ...
    def _get_lr_scale(self) -> float:
        ...
        # Both terms from the paper formula, pick whichever is smaller
        decay_term  = step ** (-0.5)
        warmup_term = step * (self.warmup_steps ** (-1.5))
 
        return (self.d_model ** (-0.5)) * min(decay_term, warmup_term)
